...
import numpy as np
import os
file_path = os.path.join(os.path.dirname(__file__), "data_set_asiap_pwt.xlsx")
...
print("This code analyzes (Europe) oecd countries' average annual real gdp per capita growth rate.\n")
...
def draw_plot():
    ...
    plt.xlabel("2000-2019 Average Annual RGDP per Capita Growth Rate(%)", size=16)
    plt.ylabel("European OECD Countries", size=16)
    plt.bar_label(p.containers[0])
    plt.title("Comparing Average Annual RGDP per Capita Growth Rate of European OECD Countries 2000-2019")
